...
```{R}
pacman::p_load(tidyverse, sf, tmap, maptools, spatstat, spNetwork, classInt, viridis, arrow,dplyr,lubridate,sparr,raster)
```
...
```{R}
G_1 <- Gest(acled_quarter_ppp_jit,correction = "border")
plot(G_1,xlim=c(0,500))
```
### 7.2 Computing F-function estimation
```{R}
f_1 <- Fest(acled_quarter_ppp_jit)
plot(f_1)
```
